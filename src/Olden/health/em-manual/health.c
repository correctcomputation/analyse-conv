...
_Ptr<struct Village> alloc_tree(int level, int label, _Ptr<struct Village> back) {
  if (level == 0)
    ...
    int i;
    _Ptr<struct Village> fval _Checked[4] = { ((void*)0), ((void*)0), ((void*)0), ((void*)0) };
...
int main(int argc, _Array_ptr<_Nt_array_ptr<char>> argv : count(argc))
{
  ...
  _Ptr<struct Village> top = ((void*)0);
  int i;
  ...
  _Unchecked { printf("Working...\n"); };

  for (i = 0; i < max_time; i++) {
    if ((i % 50) == 0) _Unchecked { printf("%d\n", i); };
    sim(top);
  }

  _Unchecked { printf("Getting Results\n"); };
  ...
  _Unchecked { printf("# of people treated:              %f people\n", total_patients); };

  _Unchecked { printf("Average length of stay:           %0.2f time units\n", total_time / total_patients); };

  _Unchecked { printf("Average # of hospitals visited:   %f hospitals\n\n", total_hosps / total_patients); };
...
_Ptr<struct List> sim(_Ptr<struct Village> village)
{
  ...
  _Ptr<struct List> val _Checked[4] = {((void*)0), ((void*)0), ((void*)0), ((void*)0)};
