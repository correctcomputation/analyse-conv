...
static BlueReturn BlueRule(Vertex inserted, Vertex vlist)
{
  ...
  else _Unchecked { (printf)("Not found\n"); };
...
static int ComputeMst(Graph graph,int numproc,int numvert)
{
  ...
  _Unchecked { (printf)("Compute phase 1\n"); };
  ...
  _Unchecked { (printf)("Compute phase 2\n"); };
  while (numvert)
    {
      BlueReturn br = {0};
...
int main(int argc, _Array_ptr<_Nt_array_ptr<char>> argv : count(argc))
{
  ...
  _Unchecked { (printf)("Making graph of size %d\n",size); };

  graph = MakeGraph(size,NumNodes);
  _Unchecked { (printf)("Graph completed\n"); };

  _Unchecked { (printf)("About to compute mst \n"); };

  dist = ComputeMst(graph,NumNodes,size);

  _Unchecked { (printf)("MST has cost %d\n",dist); };
  exit(0);
}
