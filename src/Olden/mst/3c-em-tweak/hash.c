...
Hash MakeHash(int size, _Ptr<int (unsigned int)> map)
{
  ...
  retval = (_Ptr<struct hash>) malloc<struct hash>(sizeof(*retval));
  retval->array = (_Array_ptr<HashEntry>) malloc<_Ptr<HashEntry>>(size*sizeof(HashEntry));
  for (i=0; i<size; i++)
...
void HashInsert(void *entry, unsigned int key, Hash hash)
{
  ...
  j = (hash->mapfunc)(key);
  ent = (_Ptr<struct hash_entry>) malloc<struct hash_entry>(sizeof(*ent));
