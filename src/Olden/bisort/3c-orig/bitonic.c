...
void InOrder(_Ptr<HANDLE> h) {
  _Ptr<HANDLE> l = ((void *)0);
  _Ptr<HANDLE> r = ((void *)0);
  if ((h != ((HANDLE *) 0))) {
    l = h->left;
    r = h->right;
    InOrder(l);
...
future_cell_int f_right = {};

  _Ptr<HANDLE> h = ((void *)0);
  my_name=foo++;
  if (n > 1) {
    int newnode;
    if (level<NDim)
    ...
    seed = my_random(seed);
    next_val=seed % 100;
    { h = (_Ptr<HANDLE>) malloc<HANDLE>(sizeof(struct node)); h->value = next_val; h->left = ((_Ptr<HANDLE>) 0); h->right = ((_Ptr<HANDLE>) 0); };;
...
{ int rightexchange;
  int elementexchange;
  _Ptr<HANDLE> pl = ((void *)0);
  _Ptr<HANDLE> pll = ((void *)0);
  _Ptr<HANDLE> plr = ((void *)0);
  _Ptr<HANDLE> pr = ((void *)0);
_Ptr<HANDLE> prl = ((void *)0);
_Ptr<HANDLE> prr = ((void *)0);

  _Ptr<HANDLE> rl = ((void *)0);
  _Ptr<HANDLE> rr = ((void *)0);
  int rv,lv;
