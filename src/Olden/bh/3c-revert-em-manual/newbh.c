...
int error(_Nt_array_ptr<char> msg : count(28));
...
void my_free(nodeptr n)
{
  if (((n)->type) == 01) {
    bodyptr p = 0;
     { p = (_Ptr<struct bnode>)n; }
    ((p)->next) = bp_free_list;
    bp_free_list = p;
  }
  else {
    cellptr p = 0;
     { p = (_Ptr<struct cnode>)n; }
    ((p)->next) = cp_free_list;
    cp_free_list = p;
  }
}
...
{ register bodyptr tmp = ((void*)0);
  ...
  tmp = malloc<struct bnode>(sizeof(body));
...
{ register cellptr tmp = ((void*)0);
  ...
  if (cp_free_list != ((void*)0)) {
    ...
    {
      tmp = malloc<struct cnode>(sizeof(cell));
    }
  ((tmp)->type) = 02;
  ((tmp)->proc) = p;
  for (i=0; i < (1 << 3); i++)
...
nodeptr loadtree(bodyptr p, icstruct xpic, nodeptr t, int l, treeptr tr)
{
  ...
  if (t == ((void*)0))
    ...
    if (((t)->type) == 01) {
      ...
      ((c)->subp)[si] = (_Ptr<struct node>) t;
      t = (_Ptr<struct node>) c;
    }
