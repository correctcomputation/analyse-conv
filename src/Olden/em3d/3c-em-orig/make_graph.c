...
void make_neighbors(_Ptr<node_t> nodelist, _Array_ptr<_Array_ptr<_Ptr<node_t>>> table : count(1), int tablesz, int degree, int percent_local, int id)
{
  _Ptr<node_t> cur_node = ((void *)0);
...
void make_all_neighbors(_Ptr<table_t> table, int groupname) {
  ...
  _Array_ptr<_Ptr<node_t>> local_table : count(1) = ((void *)0);
  _Array_ptr<_Array_ptr<_Ptr<node_t>>> local_table_array : count(1) = ((void *)0);
...
_Ptr<graph_t> initialize_graph(void) {
  ...
  for (i=0; i<NumNodes; i++) {
    _Array_ptr<_Ptr<node_t>> local_table : count(j) = table->e_table[i*blocksize];
