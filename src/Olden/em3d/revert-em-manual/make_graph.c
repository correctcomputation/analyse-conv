...
void fill_table(node_t * * node_table , double * values , int size, int procname)
{
  ...
  *values = gen_uniform_double();
  ...
  prev_node->from_count = 0,
    prev_node->from_values = (double * *)prev_node ->from_values,
    prev_node->coeffs = (double *)prev_node ->coeffs;
  ...
  for (i=1; i<size; i++) {
    ...
    *values = gen_uniform_double();
    ...
    cur_node->from_count = 0,
      cur_node->from_values = (double * *)cur_node ->from_values,
      cur_node->coeffs = (double *)cur_node ->coeffs;
 ...
 node_t * * tmp2  = 0;
  { tmp2 = tmp; }
      ...
      ++other_node->from_count,
          other_node->from_values = (double * *)other_node ->from_values,
          other_node->coeffs = (double *)other_node ->coeffs;
    }
  }
}
