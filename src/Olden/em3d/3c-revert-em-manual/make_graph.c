...
_Array_ptr<_Ptr<node_t>> make_table(int size, int procname) : count(size)  {
  _Array_ptr<_Ptr<node_t>> retval : count(size)  = calloc<_Ptr<node_t>>(size, sizeof(node_t *));
...
void fill_table(_Array_ptr<_Ptr<node_t>> node_table : count(size), _Array_ptr<double> values, int size, int procname)
{
  ...
  *values = gen_uniform_double();
  ...
  prev_node->from_count = 0,
    prev_node->from_values = (_Array_ptr<_Ptr<double>>)prev_node ->from_values,
    prev_node->coeffs = (_Array_ptr<double>)prev_node ->coeffs;
  ...
  for (i=1; i<size; i++) {
    ...
    *values = gen_uniform_double();
    ...
    cur_node->from_count = 0,
      cur_node->from_values = (_Array_ptr<_Ptr<double>>)cur_node ->from_values,
      cur_node->coeffs = (_Array_ptr<double>)cur_node ->coeffs;
...
void make_neighbors(_Ptr<node_t> nodelist, _Array_ptr<table_arr_t> table : count(1), int tablesz, int degree, int percent_local, int id)
{
  ...
  for(; cur_node; cur_node=cur_node->next) {
    ...
    _Array_ptr<_Ptr<node_t>> tmp : count(degree)  = calloc<_Ptr<node_t>>(degree, (sizeof(node_t *)));
    ;

     { cur_node->degree = degree, cur_node->to_nodes = tmp; }

    for (j=0; j<degree; j++) {
      do {
        _Array_ptr<_Ptr<node_t>> local_table : count(j + 1)  = ((void*)0);
        ...
        _Ptr<_Ptr<node_t>> ub = tmp + j;
 _Array_ptr<_Ptr<node_t>> tmp2  = 0;
  { tmp2 = tmp; }
        for ( ; tmp2 < ub; tmp2++)
      ...
      ++other_node->from_count,
          other_node->from_values = (_Array_ptr<_Ptr<double>>)other_node ->from_values,
          other_node->coeffs = (_Array_ptr<double>)other_node ->coeffs;
    }
  }
}
...
void update_from_coeffs(_Ptr<node_t> nodelist) {
  ...
  for (; cur_node; cur_node=cur_node->next) {
    ...
    if (from_count < 1) {
      ...
      cur_node->from_values = calloc<_Ptr<double>>(20, sizeof(double *));
      cur_node->coeffs = calloc<double>(20, sizeof(double));
      cur_node->from_length = 0;
    } else {
      cur_node->from_values = calloc<_Ptr<double>>(from_count, sizeof(double *));
...
void make_all_neighbors(_Ptr<table_t> table, int groupname) {
  ...
  _Array_ptr<_Ptr<node_t>> local_table : count(0 + 1)  = ((void*)0);
  _Array_ptr<table_arr_t> local_table_array  = ((void*)0);
...
_Ptr<graph_t>  initialize_graph(void) {
  ...
  for (i=0; i<NumNodes; i++) {
    ...
    int local_table_bounds = local_table_size;
    _Array_ptr<_Ptr<node_t>> local_table : count(i + 1)  = ((void*)0);
