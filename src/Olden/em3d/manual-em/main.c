...
{
  _Ptr<node_t> cur_node = graph->e_nodes[id];

  for(; cur_node; cur_node=cur_node->next)
    {
      _Unchecked { printf("E: value %f, from_count %d\n", *cur_node->value, cur_node->from_count); };

    }
  cur_node=graph->h_nodes[id];
  for(; cur_node; cur_node=cur_node->next)
    {
      _Unchecked { printf("H: value %f, from_count %d\n", *cur_node->value, cur_node->from_count); };
...
int main(int argc, _Array_ptr<_Nt_array_ptr<char>> argv : count(argc))
{
  ...
  _Unchecked { printf("Hello world--Doing em3d with args %d %d %d %d\n", n_nodes,d_nodes,local_p,NumNodes); };
