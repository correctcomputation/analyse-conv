...
BuildHCG(void)
{
 ...
 for (which = 1; which <= channelNets; which++) {
  ...
  ((void) sizeof ((add) ? 1 : 0), __extension__ ({ if (add) ; else __assert_fail ("add", "/home/matt/analyze-conversion-1.wt/src/Ptrdist/yacr2/orig-em-tmp/hcg.c", 95, __extension__ __PRETTY_FUNCTION__); }));




  ((void) sizeof ((storageLimitHCG > 0) ? 1 : 0), __extension__ ({ if (storageLimitHCG > 0) ; else __assert_fail ("storageLimitHCG > 0", "/home/matt/analyze-conversion-1.wt/src/Ptrdist/yacr2/orig-em-tmp/hcg.c", 100, __extension__ __PRETTY_FUNCTION__); }));
