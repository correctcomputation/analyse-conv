...
AddWords(void) {
    ...
    while (*pch) {
            ...
            || pch[1] == cchPhraseLength)
        {
            ...
            ;
             { BuildWord(_Assume_bounds_cast<_Nt_array_ptr<char>>(pch+2, byte_count(0)), _Assume_bounds_cast<_Ptr<char>>(wordStart), _Assume_bounds_cast<_Ptr<char>>(wordEnd)); }
        }
        pch += *pch;
    }
...
_Ptr<char> GetPhrase(_Array_ptr<char> pch : count(size), int size)
     {
