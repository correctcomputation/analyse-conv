...
ReadNetList(char *fname)
{
 ...
 { (head = prev = (Module *)malloc(sizeof(Module))); if (!(prev != ((void*)0))) { fprintf(stderr, "(%s:%s():%d): ", "/home/matt/analyze-conversion/src/Ptrdist/ks/tmp-em-orig/KS-1.c", "ReadData", 65); fprintf(stderr, "unable to allocate a module list node", 0, 0, 0); fprintf(stderr, "\n"); exit(1); } };
 ...
 while ((tok = strtok(((void*)0), " \t\n")) != ((void*)0)) {
     { (node = (Module *)malloc(sizeof(Module))); if (!(node != ((void*)0))) { fprintf(stderr, "(%s:%s():%d): ", "/home/matt/analyze-conversion/src/Ptrdist/ks/tmp-em-orig/KS-1.c", "ReadData", 72); fprintf(stderr, "unable to allocate a module list node", 0, 0, 0); fprintf(stderr, "\n"); exit(1); } };
...
NetsToModules(void)
{
 ...
 for (modNode = nets[net]; modNode != ((void*)0); modNode = (*modNode).next) {
     { (netNode = (Net *)malloc(sizeof(Net))); if (!(netNode != ((void*)0))) { fprintf(stderr, "(%s:%s():%d): ", "/home/matt/analyze-conversion/src/Ptrdist/ks/tmp-em-orig/KS-1.c", "NetsToModules", 98); fprintf(stderr, "unable to allocate net list node", 0, 0, 0); fprintf(stderr, "\n"); exit(1); } };
...
InitLists(void)
{
 ...
 { (mr = (ModuleRec *)malloc(sizeof(ModuleRec))); if (!(mr != ((void*)0))) { fprintf(stderr, "(%s:%s():%d): ", "/home/matt/analyze-conversion/src/Ptrdist/ks/tmp-em-orig/KS-1.c", "main", 147); fprintf(stderr, "unable to allocate ModuleRec", 0, 0, 0); fprintf(stderr, "\n"); exit(1); } };
 ...
 { (mr = (ModuleRec *)malloc(sizeof(ModuleRec))); if (!(mr != ((void*)0))) { fprintf(stderr, "(%s:%s():%d): ", "/home/matt/analyze-conversion/src/Ptrdist/ks/tmp-em-orig/KS-1.c", "main", 166); fprintf(stderr, "unable to allocate ModuleRec", 0, 0, 0); fprintf(stderr, "\n"); exit(1); } };
