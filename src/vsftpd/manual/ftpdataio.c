...
int
vsf_ftpdataio_transfer_dir(_Ptr<struct vsf_session> p_sess, int is_control, _Ptr<struct vsf_sysutil_dir> p_dir, _Ptr<const struct mystr> p_base_dir_str, _Ptr<const struct mystr> p_option_str, _Ptr<const struct mystr> p_filter_str, int is_verbose)
{
...
static int
write_dir_list(_Ptr<struct vsf_session> p_sess, _Ptr<struct mystr_list> p_dir_list, enum EVSFRWTarget target)
{
...
vsf_ftpdataio_transfer_file(_Ptr<struct vsf_session> p_sess, int remote_fd, int file_fd, int is_recv, int is_ascii)
{
  if (!is_recv)
  {
...
do_file_send_rwloop(_Ptr<struct vsf_session> p_sess, int file_fd, int is_ascii)
{
  ...
  while (1)
  {
    ...
    _Array_ptr<char> tmp : count(chunk_size) = _Dynamic_bounds_cast<_Array_ptr<char>>(p_readbuf, count(chunk_size));
    int retval = vsf_sysutil_read<char>(file_fd, tmp, chunk_size);
    if (vsf_sysutil_retval_is_error(retval))
    {
      ret_struct.retval = -1;
      return ret_struct;
    ...
    if (is_ascii)
    {
      _Array_ptr<const char> read_tmp : count(retval) = _Dynamic_bounds_cast<_Array_ptr<const char>>(p_readbuf,  count(retval));
      _Array_ptr<char> ascii_tmp : count(retval * 2) = _Dynamic_bounds_cast<_Array_ptr<char>>(p_asciibuf, byte_count(retval * 2));
      struct bin_to_ascii_ret ret =
          vsf_ascii_bin_to_ascii(read_tmp, ascii_tmp,
                                 (unsigned int) retval,
                                 prev_cr);
    ...
    _Array_ptr<const char> write_tmp : byte_count(num_to_write) = _Dynamic_bounds_cast<_Array_ptr<const char>>(p_writefrom_buf,  byte_count(num_to_write));
...
static struct vsf_transfer_ret
do_file_send_sendfile(_Ptr<struct vsf_session> p_sess, int net_fd, int file_fd, filesize_t curr_file_offset, filesize_t bytes_to_send)
{
  int retval;
...
{
  static _Array_ptr<char> p_recvbuf : count(VSFTP_DATA_BUFSIZE + 1);
  unsigned int num_to_write;
  struct vsf_transfer_ret ret_struct = { 0, 0 };
  unsigned int chunk_size = get_chunk_size();
  int prev_cr = 0;
  ...
  while (1)
  {
    ...
    _Array_ptr<char> recv_tmp : byte_count(chunk_size) = p_recvbuf + 1;
