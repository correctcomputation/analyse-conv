...
static void ftp_write_text_common(_Ptr<struct vsf_session> p_sess, int status, _Nt_array_ptr<const char> p_text : count(0), char sep);
...
void
vsf_cmdio_write(_Ptr<struct vsf_session> p_sess, int status, _Nt_array_ptr<const char> p_text : count(0))
{
  ftp_write_text_common(p_sess, status, p_text, ' ');
}

void
vsf_cmdio_write_hyphen(_Ptr<struct vsf_session> p_sess, int status, _Nt_array_ptr<const char> p_text : count(0))
{
  ftp_write_text_common(p_sess, status, p_text, '-');
}

void
vsf_cmdio_write_raw(_Ptr<struct vsf_session> p_sess, _Nt_array_ptr<const char> p_text : count(0))
{
...
void
vsf_cmdio_write_exit(_Ptr<struct vsf_session> p_sess, int status, _Nt_array_ptr<const char> p_text : count(0), int exit_val)
{
...
static void
ftp_write_text_common(_Ptr<struct vsf_session> p_sess, int status, _Nt_array_ptr<const char> p_text : count(0), char sep)
{
  /* XXX - could optimize */
  static struct mystr s_the_str;
  str_alloc_text(&s_the_str, p_text);
...
void
vsf_cmdio_write_str_hyphen(_Ptr<struct vsf_session> p_sess, int status, _Ptr<const struct mystr> p_str)
{
  ftp_write_str_common(p_sess, status, '-', p_str);
}

void
vsf_cmdio_write_str(_Ptr<struct vsf_session> p_sess, int status, _Ptr<const struct mystr> p_str)
{
  ftp_write_str_common(p_sess, status, ' ', p_str);
}

static void
ftp_write_str_common(_Ptr<struct vsf_session> p_sess, int status, char sep, _Ptr<const struct mystr> p_str)
{
...
void
vsf_cmdio_set_alarm(_Ptr<struct vsf_session> p_sess)
{
...
void
vsf_cmdio_get_cmd_and_arg(_Ptr<struct vsf_session> p_sess, _Ptr<struct mystr> p_cmd_str, _Ptr<struct mystr> p_arg_str, int set_alarm)
{
  int ret;
...
static int
control_getline(_Ptr<struct mystr> p_str, _Ptr<struct vsf_session> p_sess)
{
  int ret;
