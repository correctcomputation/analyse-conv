...
static void handle_sigill(_Ptr<void> duff);
...
vsf_standalone_main(void)
{
  ...
  else
  {
    ...
    else
    {
      ...
      _Array_ptr<const unsigned char> p_raw_addr : byte_count(sizeof(struct in6_addr)) = ((void *)0);
      ...
      if (str_getlen(&scope_id) > 0) {
        _Unchecked {
          if_index = if_nametoindex((char*) str_getbuf(&scope_id));
        }
        str_free(&scope_id);
      }
...
drop_ip_count(_Ptr<void> p_raw_addr)
{
  ...
  _Unchecked {
    p_count = _Assume_bounds_cast<_Ptr<unsigned int>>(hash_lookup_entry(s_p_ip_count_hash, (void*) p_raw_addr));
  }
  if (!p_count)
  {
...
handle_sigchld(_Ptr<void> duff)
{
  ...
  while (reap_one)
  {
    ...
    if (reap_one)
    {
      _Ptr<struct vsf_sysutil_ipaddr> p_ip = 0;
      /* Account total number of instances */
      --s_children;
      /* Account per-IP limit */
      _Unchecked {
        p_ip = _Assume_bounds_cast<_Ptr<struct vsf_sysutil_ipaddr>>(
...
handle_ip_count(_Ptr<void> p_ipaddr)
{
  _Ptr<unsigned int> p_count = 0;
  _Unchecked {
    p_count = _Assume_bounds_cast<_Ptr<unsigned int>>(hash_lookup_entry(s_p_ip_count_hash, (void*) p_ipaddr));
  }
  ...
  {
    count = 1;
    hash_add_entry(s_p_ip_count_hash,  p_ipaddr, &count);
  }
  else
  {
    count = *p_count;
