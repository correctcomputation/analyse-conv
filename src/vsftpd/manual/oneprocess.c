...
int
vsf_one_process_get_priv_data_sock(_Ptr<struct vsf_session> p_sess)
{
...
void
vsf_one_process_pasv_cleanup(_Ptr<struct vsf_session> p_sess)
{
  vsf_privop_pasv_cleanup(p_sess);
}

int
vsf_one_process_pasv_active(_Ptr<struct vsf_session> p_sess)
{
  return vsf_privop_pasv_active(p_sess);
}

unsigned short
vsf_one_process_listen(_Ptr<struct vsf_session> p_sess)
{
  return vsf_privop_pasv_listen(p_sess);
}

int
vsf_one_process_get_pasv_fd(_Ptr<struct vsf_session> p_sess)
{
  return vsf_privop_accept_pasv(p_sess);
}

void
vsf_one_process_chown_upload(_Ptr<struct vsf_session> p_sess, int fd)
{
