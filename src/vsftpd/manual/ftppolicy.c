...
static int socket_validator(_Ptr<struct pt_sandbox> p_sandbox, _Ptr<void> p_arg);
static int connect_validator(_Ptr<struct pt_sandbox> p_sandbox, _Ptr<void> p_arg);
static int getsockopt_validator(_Ptr<struct pt_sandbox> p_sandbox, _Ptr<void> p_arg);
static int setsockopt_validator(_Ptr<struct pt_sandbox> p_sandbox, _Ptr<void> p_arg);

void
policy_setup(_Ptr<struct pt_sandbox> p_sandbox, _Ptr<const struct vsf_session> p_sess)
{
...
static int
socket_validator(_Ptr<struct pt_sandbox> p_sandbox, _Ptr<void> p_arg)
{
  int ret;
...
static int
connect_validator(_Ptr<struct pt_sandbox> p_sandbox, _Ptr<void> p_arg)
{
  int ret;
  _Ptr<struct vsf_session> p_sess = 0;
  _Unchecked {
  ...
  static _Ptr<struct vsf_sysutil_sockaddr> p_sockptr;
  if (vsf_sysutil_sockaddr_is_ipv6(p_sess->p_local_addr))
  {
    expected_family = AF_INET6;
  ...
  p_buf = vsf_sysutil_malloc<void>((int) expected_len);
  ...
  _Unchecked {
    p_sockaddr = _Assume_bounds_cast<_Ptr<struct sockaddr>>(p_buf);
  }
  ...
  {
    _Ptr<struct sockaddr_in> p_sockaddr_in = 0;
    _Unchecked {
      p_sockaddr_in = _Assume_bounds_cast<_Ptr<struct sockaddr_in>>(p_sockaddr);
    }
  ...
  {
    _Ptr<struct sockaddr_in6> p_sockaddr_in6 = 0;
    _Unchecked {
      p_sockaddr_in6 = _Assume_bounds_cast<_Ptr<struct sockaddr_in6>>(p_sockaddr);
    }
...
static int
getsockopt_validator(_Ptr<struct pt_sandbox> p_sandbox, _Ptr<void> p_arg)
{
  int ret;
  unsigned long arg2;
  unsigned long arg3;
...
static int
setsockopt_validator(_Ptr<struct pt_sandbox> p_sandbox, _Ptr<void> p_arg)
{
  int ret;
  unsigned long arg2;
  unsigned long arg3;
