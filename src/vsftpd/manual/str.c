...
static void str_split_text_common(_Ptr<struct mystr> p_src, _Ptr<struct mystr> p_rhs, _Nt_array_ptr<const char> p_text : count(0), int is_reverse);
static int str_equal_internal(_Array_ptr<const char> p_buf1 : count(buf1_len), unsigned int buf1_len, _Array_ptr<const char> p_buf2 : count(buf2_len), unsigned int buf2_len);

/* Private functions */

static void
s_setbuf(_Ptr<struct mystr> p_str, _Array_ptr<char> p_newbuf : count(p_str->alloc_bytes + 1))
{
  if (p_str->p_buf != 0)
  {
    bug("p_buf not NULL when setting it");
  }
  p_newbuf[p_str->alloc_bytes] = '\0';
  _Unchecked {
    p_str->p_buf = (_Nt_array_ptr<char>) p_newbuf;
  }
}

void
private_str_alloc_memchunk(_Ptr<struct mystr> p_str, _Array_ptr<const char> p_src : byte_count(len), unsigned int len)
{
  ...
  {
    str_free(p_str);
    s_setbuf(p_str, vsf_sysutil_malloc<char>(buf_needed + 1));
    p_str->alloc_bytes = buf_needed;
  }
  vsf_sysutil_memcpy<char>(p_str->p_buf, p_src, len);
  p_str->p_buf[len] = '\0';
  p_str->len = len;
}

void
private_str_append_memchunk(_Ptr<struct mystr> p_str, _Array_ptr<const char> p_src : byte_count(len), unsigned int len)
{
  unsigned int buf_needed;
  ...
  if (buf_needed > p_str->alloc_bytes)
  {
    _Array_ptr<char> tmp : count (buf_needed + 1) = vsf_sysutil_realloc<char>(p_str->p_buf, buf_needed + 1);
    tmp[buf_needed] = '\0';
    _Nt_array_ptr<char> tmp_nt : count(buf_needed) = 0;
    _Unchecked {
      tmp_nt = (_Nt_array_ptr<char>) tmp;
    }
    p_str->p_buf = tmp_nt;
    p_str->alloc_bytes = buf_needed;
  }
  vsf_sysutil_memcpy<char>(p_str->p_buf + p_str->len, p_src, len);
...
str_alloc_text(_Ptr<struct mystr> p_str, _Nt_array_ptr<const char> p_src : count(0))
{
  ...
  unchecked_widen_bounds(_Nt_array_ptr<const char>, tmp, count(len), p_src)
...
_Nt_array_ptr<const char> str_strdup_trimmed(_Ptr<const struct mystr> p_str)
{
  _Nt_array_ptr<const char> p_trimmed : count(p_str->len) = p_str->p_buf;
  int h, t, newlen;

  for (h = 0; h < p_str->len && vsf_sysutil_isspace(p_trimmed[h]); h++) ;
  for (t = p_str->len - 1; t >= 0 && vsf_sysutil_isspace(p_trimmed[t]); t--) ;
  newlen = t - h + 1;
  return newlen ? vsf_sysutil_strndup(p_trimmed+h, (unsigned int)newlen) : 0L;
}

void
str_alloc_alt_term(_Ptr<struct mystr> p_str, _Array_ptr<const char> p_src : count(src_len), char term, unsigned int src_len)
{
  unsigned int len = 0;
  while (len < src_len && p_src[len] != term)
  {
    len++;
    if (len == 0)
    {
  ...
  _Array_ptr<const char> tmp : count(len) = _Dynamic_bounds_cast<_Array_ptr<const char>>(p_src, count(len));
  private_str_alloc_memchunk(p_str, tmp, len);
}
...
str_reserve(_Ptr<struct mystr> p_str, unsigned int res_len)
{
  ...
  {
    _Array_ptr<char> tmp : count(res_len + 1) = vsf_sysutil_realloc<char>(p_str->p_buf, res_len + 1);
    tmp[res_len] = '\0';
    _Unchecked {
      p_str->p_buf = (_Nt_array_ptr<char>) tmp;
    }
...
{
  return str_equal_internal(p_str1->p_buf, p_str1->len, p_str2->p_buf, p_str2->len);
}

static int
str_equal_internal(_Array_ptr<const char> p_buf1 : count(buf1_len), unsigned int buf1_len, _Array_ptr<const char> p_buf2 : count(buf2_len), unsigned int buf2_len)
{
  int retval;
  ...
  _Array_ptr<const char> tmp1 : count(minlen) = _Dynamic_bounds_cast<_Array_ptr<const char>>(p_buf1, count(minlen));
  _Array_ptr<const char> tmp2 : count(minlen) = _Dynamic_bounds_cast<_Array_ptr<const char>>(p_buf2, count(minlen));
  retval = vsf_sysutil_memcmp<const char>(tmp1, tmp2, minlen);
...
str_equal_text(_Ptr<const struct mystr> p_str, _Nt_array_ptr<const char> p_text : count(0))
{
  ...
  unchecked_widen_bounds(_Nt_array_ptr<const char>, tmp_text, count(cmplen), p_text)
  _Nt_array_ptr<const char> tmp_buf : count(p_str->len) = p_str->p_buf;
  return (str_equal_internal(tmp_buf, p_str->len, tmp_text, cmplen) == 0);
}
...
str_append_text(_Ptr<struct mystr> p_str, _Nt_array_ptr<const char> p_src : count(0))
{
  unsigned int len = vsf_sysutil_strlen(p_src);
  unchecked_widen_bounds(_Nt_array_ptr<const char>, tmp, count(len), p_src)
  private_str_append_memchunk(p_str, tmp, len);
}
...
void
str_replace_text(_Ptr<struct mystr> p_str, _Nt_array_ptr<const char> p_from : count(0), _Nt_array_ptr<const char> p_to : count(0))
{
...
void
str_split_text(_Ptr<struct mystr> p_src, _Ptr<struct mystr> p_rhs, _Nt_array_ptr<const char> p_text : count(0))
{
  str_split_text_common(p_src, p_rhs, p_text, 0);
}

void
str_split_text_reverse(_Ptr<struct mystr> p_src, _Ptr<struct mystr> p_rhs, _Nt_array_ptr<const char> p_text : count(0))
{
  str_split_text_common(p_src, p_rhs, p_text, 1);
}

static void
str_split_text_common(_Ptr<struct mystr> p_src, _Ptr<struct mystr> p_rhs, _Nt_array_ptr<const char> p_text : count(0), int is_reverse)
{
  ...
  /* Build rhs */
  private_str_alloc_memchunk(p_rhs, p_src->p_buf + indexx + search_len, p_src->len - indexx - search_len);
...
struct str_locate_result
str_locate_str_reverse(_Ptr<const struct mystr> p_str, _Ptr<const struct mystr> p_look_str)
{
...
struct str_locate_result
str_locate_chars(_Ptr<const struct mystr> p_str, _Nt_array_ptr<const char> p_chars : count(0))
{
...
struct str_locate_result
str_locate_text(_Ptr<const struct mystr> p_str, _Nt_array_ptr<const char> p_text : count(0))
{
  struct str_locate_result retval;
  unsigned int i;
  unsigned int text_len = vsf_sysutil_strlen(p_text);

  unchecked_widen_bounds(_Nt_array_ptr<const char>, p_text_bounds, count(text_len), p_text)
  ...
  {
    if (vsf_sysutil_memcmp<char>(p_str->p_buf + i, p_text_bounds, text_len) == 0)
    {
      retval.found = 1;
      retval.index = i;
      return retval;
...
struct str_locate_result
str_locate_text_reverse(_Ptr<const struct mystr> p_str, _Nt_array_ptr<const char> p_text : count(0))
{
  struct str_locate_result retval;
  unsigned int i;
  unsigned int text_len = vsf_sysutil_strlen(p_text);
...
void
str_mid_to_end(_Ptr<const struct mystr> p_str, _Ptr<struct mystr> p_out, unsigned int indexx)
{
  if (indexx > p_str->len)
  {
    bug("invalid indexx in str_mid_to_end");
  }
  private_str_alloc_memchunk(p_out, p_str->p_buf + indexx, p_str->len - indexx);
}
...
int
str_getline(_Ptr<const struct mystr> p_str, _Ptr<struct mystr> p_line_str, _Ptr<unsigned int> p_pos)
{
  unsigned int start_pos = *p_pos;
  unsigned int curr_pos = start_pos;
  unsigned int buf_len = str_getlen(p_str);
  _Nt_array_ptr<const char> p_buf : count(buf_len) = p_str->p_buf;
  ...
  _Nt_array_ptr<const char> tmp0 : count(buf_len - start_pos) = _Dynamic_bounds_cast<_Nt_array_ptr<const char>>(p_buf + start_pos, count(buf_len - start_pos));
  _Nt_array_ptr<const char> tmp1 : count(out_len) = _Dynamic_bounds_cast<_Nt_array_ptr<const char>>(tmp0, count(out_len));
  private_str_alloc_memchunk(p_line_str, tmp1, out_len);
  *p_pos = curr_pos;
  return 1;
}
...
void
str_basename (_Ptr<struct mystr> d_str, _Ptr<const struct mystr> path)
{
