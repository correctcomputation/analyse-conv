...
struct mystr
{
  _Nt_array_ptr<char> PRIVATE_HANDS_OFF_p_buf : count(PRIVATE_HANDS_OFF_alloc_bytes);
...
#endif
void private_str_alloc_memchunk(_Ptr<struct mystr> p_str, _Array_ptr<const char> p_src : byte_count(len), unsigned int len);
...
void str_alloc_alt_term(_Ptr<struct mystr> p_str, _Array_ptr<const char> p_src : count(len), char term, unsigned int len);
...
void str_replace_text(_Ptr<struct mystr> p_str, _Nt_array_ptr<const char> p_from : count(0), _Nt_array_ptr<const char> p_to : count(0));
...
void str_split_text(_Ptr<struct mystr> p_src, _Ptr<struct mystr> p_rhs, _Nt_array_ptr<const char> p_text : count(0));
void str_split_text_reverse(_Ptr<struct mystr> p_src, _Ptr<struct mystr> p_rhs, _Nt_array_ptr<const char> p_text : count(0));
...
struct str_locate_result str_locate_text(_Ptr<const struct mystr> p_str, _Nt_array_ptr<const char> p_text : count(0));
struct str_locate_result str_locate_text_reverse(_Ptr<const struct mystr> p_str, _Nt_array_ptr<const char> p_text : count(0));
struct str_locate_result str_locate_chars(_Ptr<const struct mystr> p_str, _Nt_array_ptr<const char> p_chars : count(0));
...
void str_basename (_Ptr<struct mystr> d_str, _Ptr<const struct mystr> path);
