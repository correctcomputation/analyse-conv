...
void
ssl_slave(_Ptr<struct vsf_session> p_sess)
{
  ...
  while (1)
  {
    ...
    {
      ret = ftp_getline(p_sess, &p_sess->ftp_cmd_str, p_sess->p_control_line_buf);
    ...
    else if (cmd == PRIV_SOCK_DO_SSL_WRITE)
    {
      ...
      unchecked_widen_bounds(_Nt_array_ptr<char>, tmp, count(len), str_getbuf(&data_str))
      ret = ssl_write(p_sess->p_data_ssl, tmp, len);
