...
_Ptr<struct hash> hash_alloc(unsigned int buckets, unsigned int key_size, unsigned int value_size, hashfunc_t hash_func)
{
  unsigned int size;
  ...
  p_hash->p_nodes = vsf_sysutil_malloc<_Ptr<struct hash_node>>(size);
  vsf_sysutil_memclr<_Ptr<struct hash_node>>(p_hash->p_nodes, size);
  return p_hash;
}
...
hash_add_entry(_Ptr<struct hash> p_hash, _Ptr<void> p_key, _Ptr<void> p_value)
{
  ...
  vsf_sysutil_memcpy<void>(p_new_node->p_key, p_key, p_hash->key_size);
  p_new_node->p_value = vsf_sysutil_malloc<void>(p_hash->value_size);
  vsf_sysutil_memcpy<void>(p_new_node->p_value, p_value, p_hash->value_size);
...
_Ptr<struct hash_node> hash_get_node_by_key(_Ptr<struct hash> p_hash, _Ptr<void> p_key)
{
  ...
  while (p_node != 0 &&
         vsf_sysutil_memcmp<void>(p_key, p_node->p_key, p_hash->key_size) != 0)
  {
