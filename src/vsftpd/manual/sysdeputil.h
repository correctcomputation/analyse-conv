...
int vsf_sysdep_check_auth(_Ptr<struct mystr> p_user, _Ptr<const struct mystr> p_pass, _Ptr<const struct mystr> p_remote_host);
 ...
 * have any effect.
 */
void vsf_sysutil_setproctitle_init(int argc, _Array_ptr<_Nt_array_ptr<const char>> argv : count(argc));
void vsf_sysutil_setproctitle(_Nt_array_ptr<const char> p_text : count(0));
