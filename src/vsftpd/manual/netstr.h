...
typedef _Ptr<int (_Ptr<struct vsf_session>, _Array_ptr<char> : count(l), unsigned int l)> str_netfd_read_t;
 ...
 */
int str_netfd_alloc(_Ptr<struct vsf_session> p_sess, _Ptr<struct mystr> p_str, char term, _Array_ptr<char> p_readbuf : count(maxlen), unsigned int maxlen, str_netfd_read_t p_peekfunc, str_netfd_read_t p_readfunc);
