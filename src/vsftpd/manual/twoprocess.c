...
static void calculate_chdir_dir(int anon, _Ptr<struct mystr> p_userdir_str, _Ptr<struct mystr> p_chroot_str, _Ptr<struct mystr> p_chdir_str, _Ptr<const struct mystr> p_user_str, _Ptr<const struct mystr> p_orig_user_str);
...
void
vsf_two_process_login(_Ptr<struct vsf_session> p_sess, _Ptr<const struct mystr> p_pass_str)
{
  char result;
...
handle_per_user_config(_Ptr<const struct mystr> p_user_str)
{
  ...
  if (!vsf_sysutil_retval_is_error(retval))
  {
    ...
    vsf_parseconf_load_file(((_Nt_array_ptr<const char> )str_getbuf(&filename_str)), 1);
  }
...
calculate_chdir_dir(int anon_login, _Ptr<struct mystr> p_userdir_str, _Ptr<struct mystr> p_chroot_str, _Ptr<struct mystr> p_chdir_str, _Ptr<const struct mystr> p_user_str, _Ptr<const struct mystr> p_orig_user_str)
{
  if (!anon_login)
  {
    ...
    str_alloc_text(p_userdir_str, ((_Nt_array_ptr<const char> )vsf_sysutil_user_get_homedir(p_user)));
    if (tunable_user_sub_token)
    {
      str_replace_text(p_userdir_str, tunable_user_sub_token,
                       ((_Nt_array_ptr<const char> )str_getbuf(p_orig_user_str)));
    }
  }
