...
int
str_netfd_alloc(_Ptr<struct vsf_session> p_sess, _Ptr<struct mystr> p_str, char term, _Array_ptr<char> p_readbuf : count(maxlen), unsigned int maxlen, str_netfd_read_t p_peekfunc, str_netfd_read_t p_readfunc)
{
  int retval;
  ...
  while (1)
  {
    ...
    for (i=0; i < bytes_read; i++)
    {
      if (p_readpos[i] == term)
      {
        /* Got it! */
        i++;
        _Array_ptr<char> tmp : count(i) = _Dynamic_bounds_cast<_Array_ptr<char>>(p_readpos, count(i));
        retval = (*p_readfunc)(p_sess, tmp, i);
        if (vsf_sysutil_retval_is_error(retval) ||
