...
struct ascii_to_bin_ret
{
  unsigned int stored;
  int last_was_cr;
  _Array_ptr<char> p_buf : count(stored);
};
struct ascii_to_bin_ret vsf_ascii_ascii_to_bin(_Array_ptr<char> p_in : count(in_len + 1), unsigned int in_len, int prev_cr);
...
struct bin_to_ascii_ret vsf_ascii_bin_to_ascii(_Array_ptr<const char> p_in : count(in_len), _Array_ptr<char> p_out : count(in_len * 2), unsigned int in_len, int prev_cr);
